  ...
  return {
    ...
    targetWeightKg:
      measurement === "REPS_WEIGHT" && context === "routine"
        ? numberOrNull(set.targetWeightKg)
        : null,
    targetTimeSeconds:
      measurement === "TIME" ? numberOrNull(set.targetTimeSeconds) : null,
    targetDistanceMeters:
      measurement === "DISTANCE" ? numberOrNull(set.targetDistanceMeters) : null,
    restAfterSeconds:
    ...
    rpe:
      measurement === "TIME" || measurement === "DISTANCE"
        ? null
        : numberOrNull(set.rpe),
    notes: set.notes ?? null,
    toFailure:
      measurement === "TIME" || measurement === "DISTANCE"
        ? false
        : Boolean(set.toFailure),
  }
}

export function normalizeExerciseForSubmit(
  exercise: ExerciseInBlockInput,
  orderIndex: number,
  context: "template" | "routine"
): ExerciseInBlockInput {
  ...
  return {
    ...
    sets: exercise.sets.map((set, index) =>
      normalizeSetForSubmit(set, index + 1, measurement, context)
    ),
  }
}

export function normalizeBlockForSubmit(
  block: BlockInput,
  orderIndex: number,
  context: "template" | "routine"
): BlockInput {
